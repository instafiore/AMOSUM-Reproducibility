...
from utility import PerfectHash, Group, mw, Interpretation, WeightFunction,\
    GroupFunction, not_, get_name , AggregateFunction, TrueGroupFunction, simplyLiterals
import re
...
def getLiterals(*lits):
    ...
    for a in atomNames:
        if  a.startswith('group('):
            ...
            # group(lit_name, weight, group_id, aggregate_id)
    ...
    if aggregate[l]:
        ...
        if not_(l) == mw(G):
            ...
            if true_group[G] is None :
                # NOTE: here can be none in the le
                w_n : int  = 0
                w_n = weight[new_max]
